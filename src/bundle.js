...
exports.calcImpulse = calcImpulse;
exports.calcNewBody = calcNewBody;
...
function calcNewBody(world, biggerBody, smallerBody) {
...
function addCircle(pos, bodyRadius) {
...
var _addBody = __webpack_require__(1);
...
var _interactions = __webpack_require__(5);
...
(0, _physicsjs2.default)({ sleepDisabled: true }, function (world) {
   ...
   //init rendering
   var view = new _renderer2.default();
   ...
   var behaviors = (0, _behaviors2.default)(renderer);
   world.add(behaviors);

   //menu

   (0, _menu2.default)(world);

   //interaction with the world
   world.on((0, _interactions2.default)(world));

   //collision detection for merging bodies

   (0, _collisions2.default)(world);
...
function Behaviors(renderer) {
    return [_physicsjs2.default.behavior('newtonian'), _physicsjs2.default.behavior('body-impulse-response', {
								...
								for (var i = 0; i < data.collisions.length; i++) {
												...
												if (c.overlap > c.bodyA.radius / 4 || c.overlap > c.bodyB.radius / 4) {
																if (c.bodyA.mass >= c.bodyB.mass) {
																				...
																				(0, _addBody.calcNewBody)(world, c.bodyA, c.bodyB);
																} else {
																				(0, _addBody.calcNewBody)(world, c.bodyB, c.bodyA);
																};
												}
								}
				});
...
function Interactions(world) {

    var attraction = _physicsjs2.default.behavior('attractor', {
        order: 0,
        strength: 0.001
    });
    ...
    return {
        'interact:poke': function interactPoke(pos) {
            if (!world.gravityWell) {
                ...
                var circle = (0, _addBody.addCircle)(pos, world.bodyRadius);
...
exports.default = bindMenuEvents;
function bindMenuEvents(world) {
    ...
    var huge = document.getElementById('hugeButton');
    var gravityWell = document.getElementById('gravityWellButton');
    var reset = document.getElementById('resetButton');
    var pause = document.getElementById('pauseButton');

    gravityWell.addEventListener('click', function () {
        world.gravityWell = true;
    });
    small.addEventListener('click', function () {
        world.bodyRadius = 2;world.gravityWell = false;
    });
    medium.addEventListener('click', function () {
        world.bodyRadius = 4;world.gravityWell = false;
    });
    large.addEventListener('click', function () {
        world.bodyRadius = 6;world.gravityWell = false;
    });
    huge.addEventListener('click', function () {
        world.bodyRadius = 8;world.gravityWell = false;
    });
    reset.addEventListener('click', function () {
        world.remove(world.getBodies());
    });
    pause.addEventListener('click', function () {
        world.isPaused() ? world.unpause() : world.pause();
    });
}
...
var Renderer = function () {
    function Renderer() {
        ...
        this.renderer = _physicsjs2.default.renderer('canvas', {
            el: 'viewport',
    ...
    _createClass(Renderer, [{
        key: 'resize',
        value: function resize(world) {
            this.getViewport();
            world.removeBehavior('edge-collision-detection');
            world.add(_physicsjs2.default.behavior('edge-collision-detection', {
                aabb: _physicsjs2.default.aabb(0, 0, this.viewportWidth, this.viewportHeight),
                restitution: 1,
